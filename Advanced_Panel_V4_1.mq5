...
const int PANEL_HEIGHT = 548;  // شامل ردیف جدید تشخیص روند M5/M15/M30/H1 زیر دکمه CLOSE ALL، بدون فضای اضافه
...
bool  g_trendBullish[TREND_ROW_COUNT] = {true, true, true, true};  // آخرین جهت تایید‌شده (برای جلوگیری از حالت خالی تا رسیدن دیتا)
bool  g_trendReady[TREND_ROW_COUNT]   = {false, false, false, false};
...
void CreatePanel()
{
   ...
   CreateLabel(PANEL_TITLE, "ADVANCED PANEL v4.0", g_panelX + 10, g_panelY + 5, clrGold, 9, true);
...
void CreateLabel(string name, string text, int x, int y, color clr, int fontSize, bool fontBold)
{
   ...
   ObjectSetInteger(0, name, OBJPROP_SELECTABLE, (name == PANEL_TITLE));
   ObjectSetInteger(0, name, OBJPROP_SELECTED, false);
}
...
// بازگشت false یعنی دیتای کافی هنوز آماده نیست (مثلا بلافاصله بعد از لود اکسپرت)؛ در این حالت مقدار قبلی حفظ می‌شود.
bool CalcTrendDirection(int idx, bool &bullishOut)
{
   ...
   // تایید قدرت روند: فقط وقتی ADX(14) از آستانه بیشتر باشد، جهت جدید (EMA50 نسبت به EMA200) پذیرفته و نمایش داده می‌شود.
   if(adxMain[0] <= InpTrendADXThreshold) return false; // روند به اندازه کافی قوی نیست؛ آخرین جهت تایید‌شده حفظ می‌شود

   bullishOut = (emaFast[0] > emaSlow[0]); // EMA50 بالای EMA200 = صعودی، پایین = نزولی
   return true;
}
...
void UpdateTrendRow()
{
   ...
   {
      bool bullish;
      if(CalcTrendDirection(i, bullish))
      {
         g_trendBullish[i] = bullish;
         g_trendReady[i]   = true;
      }
      ...
      string arrow = g_trendBullish[i] ? "▲" : "▼";
      color  clr   = g_trendBullish[i] ? clrLime : clrRed;
...
{
   ENUM_POSITION_TYPE checkType = (orderType == ORDER_TYPE_BUY) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
   if(HasOpenPosition(checkType)) return false;
