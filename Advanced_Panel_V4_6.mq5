...
const int PANEL_HEIGHT = 578;  // شامل جداکننده و فاصله اضافه بین بخش خط روند و بخش سفارش‌های Pending
...
void OnDeinit(const int reason)
{
   ...
   string allPanelObjects[] = {
      ...
      LBL_BE_TS_STATUS, BTN_TOGGLE_BE_TS, SEP_LINE_2
   };
...
void CreatePanel()
{
   ...
   CreateButton(BTN_TL_TOGGLE, "", g_panelX + 12, g_panelY + 284, 203, 24, C'70,70,70', clrWhite);
   UpdateTLButtonAppearance();

   CreateButton(BTN_ADD_TRENDLINE, "+ NEW TREND LINE (" + InpAutoTrendlinePrefix + ")", g_panelX + 12, g_panelY + 314, 203, 24, C'40,80,130', clrWhite);

   CreateButton(BTN_CLEAR_TRENDLINES, "CLEAR ALL TREND LINES", g_panelX + 12, g_panelY + 344, 203, 24, C'110,40,40', clrWhite);

   // جداکننده‌ی بصری کوچک بین بخش رسم خط روند و بخش سفارش‌های Pending، تا زیاد به هم نچسبند
   CreateRectLabel(SEP_LINE_2, g_panelX + 12, g_panelY + 374, 203, 1, C'80,80,80', C'80,80,80');

   CreateButton(BTN_BUY_PENDING, "BUY PENDING", g_panelX + 12, g_panelY + 382, 98, 24, C'0,90,0', clrWhite);
   CreateButton(BTN_SELL_PENDING, "SELL PENDING", g_panelX + 117, g_panelY + 382, 98, 24, C'130,0,0', clrWhite);

   CreateButton(BTN_CLOSE_BUY, "CLOSE BUYS", g_panelX + 12, g_panelY + 412, 98, 24, C'180,90,0', clrWhite);
   CreateButton(BTN_CLOSE_SELL, "CLOSE SELLS", g_panelX + 117, g_panelY + 412, 98, 24, C'180,90,0', clrWhite);

   CreateButton(BTN_CLOSE_HALF, "CLOSE 50%", g_panelX + 12, g_panelY + 442, 98, 24, C'100,100,0', clrWhite);
   CreateButton(BTN_MANUAL_BE, "MANUAL BE", g_panelX + 117, g_panelY + 442, 98, 24, C'40,100,150', clrWhite);
   
   CreateButton(BTN_CLOSE, "CLOSE ALL", g_panelX + 12, g_panelY + 472, 203, 26, C'70,70,70', clrWhite);

   // ردیف تشخیص روند M5 / M15 / M30 / H1 - دقیقا زیر دکمه CLOSE ALL (Close All bottom edge = panelY + 498)
   int trendRowY = g_panelY + 506;
   ...
   CreateLabel(LBL_BE_TS_STATUS, "No position selected", g_panelX + 12, g_panelY + 530, clrSilver, 8, false);
   CreateButton(BTN_TOGGLE_BE_TS, "TOGGLE BE/TS FOR SELECTED", g_panelX + 12, g_panelY + 546, 203, 22, C'70,70,70', clrWhite);
