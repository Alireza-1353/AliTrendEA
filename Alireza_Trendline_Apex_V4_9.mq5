//+------------------------------------------------------------------+
//|                                    Alireza_Trendline_Apex_V4_9.mq5|
//|                                  Copyright 2026, Alireza          |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Alireza Trendline Apex"
#property link      "https://www.mql5.com"
#property icon       "Alireza_Trendline_Apex.ico"
#define  EA_VERSION  "4.9"
...
string   g_gvHedgedCountKey, g_gvHedgedPrefix;   // کلیدهای GlobalVariable برای ماندگاری این لیست بین ری‌استارت‌های EA/ترمینال (رفع باگ: قبلاً فقط در حافظه بود)
...
const int PANEL_HEIGHT = 588;  // شامل جداکننده جدید زیر ردیف BUY/SELL و ضخیم‌تر شدن همه جداکننده‌ها
...
#define SEP_LINE_1      PREFIX "SepLine1"
#define SEP_LINE_2      PREFIX "SepLine2"
...
int OnInit()
{
   ...
   g_gvHedgedCountKey = PREFIX + symbol + "_HedgedCount";
   g_gvHedgedPrefix   = PREFIX + symbol + "_Hedged_";
...
void OnDeinit(const int reason)
{
   ...
   // حذف صریح تک‌تک اشیای پنل، به‌علاوه‌ی حذف عمومی بر اساس پیشوند (اطمینان مضاعف تا در حالت Remove کردن EA
   // از روی چارت، هیچ عنصری از پنل (دکمه/لیبل/باکس) به‌صورت باقیمانده روی چارت دیده نشود)
   string allPanelObjects[] = {
      ...
      LBL_BE_TS_STATUS, BTN_TOGGLE_BE_TS, SEP_LINE_1, SEP_LINE_2
   };
   for(int i = 0; i < ArraySize(allPanelObjects); i++)
      if(ObjectFind(0, allPanelObjects[i]) >= 0) ObjectDelete(0, allPanelObjects[i]);

   ObjectsDeleteAll(0, PREFIX); // پاک‌سازی عمومی نهایی برای هر شیء باقیمانده‌ای (مثل خطوط EA_1، EA_2 و ...)
   ChartRedraw();
}
...
void LoadOrInitVariables()
{
   ...
   // بازیابی لیست پوزیشن‌هایی که قبلاً توسط سیستم Auto-Hedge قفل/هج شده بودند (رفع باگ: قبلاً این لیست فقط در حافظه بود
   // و با هر بار بستن/باز کردن ترمینال پاک می‌شد، در نتیجه پوزیشن‌های قبلاً هج‌شده دوباره هج می‌شدند)
   ArrayFree(g_hedgedTickets);
   if(GlobalVariableCheck(g_gvHedgedCountKey))
   {
      int savedHedgedCount = (int)GlobalVariableGet(g_gvHedgedCountKey);
      for(int i = 0; i < savedHedgedCount; i++)
      {
         string key = g_gvHedgedPrefix + IntegerToString(i);
         if(GlobalVariableCheck(key))
         {
            ulong t = (ulong)GlobalVariableGet(key);
            if(t != 0)
...
void CreatePanel()
{
   ...
   CreateLabel(PANEL_TITLE, "TRENDLINE APEX v" + EA_VERSION, g_panelX + 10, g_panelY + 5, clrGold, 9, true);
   ...
   CreateRectLabel(PREFIX + "Line", g_panelX + 10, g_panelY + 65, w - 20, 3, C'150,150,160', C'150,150,160');

   CreateButton(BTN_MODE, "", g_panelX + 12, g_panelY + 75, 203, 22, C'60,80,120', clrWhite);
   UpdateModeButtonAppearance();

   CreateButton(CHK_SL, "", g_panelX + 12, g_panelY + 104, 22, 16, C'60,60,60', clrWhite);
   UpdateCheckboxAppearance(CHK_SL, g_useSL);
   CreateLabel(LBL_SL, "StopLoss", g_panelX + 40, g_panelY + 108, clrWhite, 8, true);
   CreateEdit(EDIT_SL, DoubleToString(g_panelSL, 1), g_panelX + 130, g_panelY + 104, 85, 16);

   CreateButton(CHK_TP, "", g_panelX + 12, g_panelY + 134, 22, 16, C'60,60,60', clrWhite);
   UpdateCheckboxAppearance(CHK_TP, g_useTP);
   CreateLabel(LBL_TP, "TakeProfit", g_panelX + 40, g_panelY + 138, clrWhite, 8, true);
   CreateEdit(EDIT_TP, DoubleToString(g_panelTP, 1), g_panelX + 130, g_panelY + 134, 85, 16);

   CreateButton(CHK_BE, "", g_panelX + 12, g_panelY + 164, 22, 16, C'60,60,60', clrWhite);
   UpdateCheckboxAppearance(CHK_BE, g_useBE);
   CreateLabel(LBL_BE, "BE / Offset", g_panelX + 40, g_panelY + 168, clrWhite, 8, true);
   CreateEdit(EDIT_BE, DoubleToString(g_panelBE, 1), g_panelX + 130, g_panelY + 164, 40, 16);
   CreateEdit(EDIT_BE_OFFSET, DoubleToString(g_panelBEOffset, 1), g_panelX + 175, g_panelY + 164, 40, 16);

   CreateButton(CHK_TS, "", g_panelX + 12, g_panelY + 194, 22, 16, C'60,60,60', clrWhite);
   UpdateCheckboxAppearance(CHK_TS, g_useTS);
   CreateLabel(LBL_TS, "TS / Step", g_panelX + 40, g_panelY + 198, clrWhite, 8, true);
   CreateEdit(EDIT_TS, DoubleToString(g_panelTS, 1), g_panelX + 130, g_panelY + 194, 40, 16);
   CreateEdit(EDIT_TS_STEP, DoubleToString(g_panelTSStep, 1), g_panelX + 175, g_panelY + 194, 40, 16);

   CreateButton(CHK_HEDGE, "", g_panelX + 12, g_panelY + 224, 22, 16, C'60,60,60', clrWhite);
   UpdateCheckboxAppearance(CHK_HEDGE, g_useHedge);
   CreateLabel(LBL_HEDGE, "Hedge Loss", g_panelX + 40, g_panelY + 228, clrWhite, 8, true);
   CreateEdit(EDIT_HEDGE, DoubleToString(g_panelHedgeLoss, 1), g_panelX + 130, g_panelY + 224, 85, 16);

   // ردیف ترکیبی BUY | LOT | SELL - دقیقا بالای دکمه EA TREND LINES (به‌جای ردیف مجزای قبلی Lot Size و ردیف مجزای Buy/Sell)
   CreateButton(BTN_BUY, "BUY", g_panelX + 12, g_panelY + 254, 78, 26, C'0,130,0', clrWhite);
   CreateLabel(LBL_LOT, "LOT", g_panelX + 103, g_panelY + 254, clrWhite, 7, true);
   CreateEdit(EDIT_LOT, DoubleToString(g_panelLot, 2), g_panelX + 93, g_panelY + 266, 40, 14);
   CreateButton(BTN_SELL, "SELL", g_panelX + 136, g_panelY + 254, 78, 26, C'170,0,0', clrWhite);

   // جداکننده‌ی بصری بین ردیف BUY/SELL و بخش رسم خط روند، تا زیاد به هم نچسبند
   CreateRectLabel(SEP_LINE_1, g_panelX + 12, g_panelY + 286, 203, 3, C'150,150,160', C'150,150,160');

   CreateButton(BTN_TL_TOGGLE, "", g_panelX + 12, g_panelY + 294, 203, 24, C'70,70,70', clrWhite);
   UpdateTLButtonAppearance();

   CreateButton(BTN_ADD_TRENDLINE, "+ NEW TREND LINE (" + InpAutoTrendlinePrefix + ")", g_panelX + 12, g_panelY + 324, 203, 24, C'40,80,130', clrWhite);

   CreateButton(BTN_CLEAR_TRENDLINES, "CLEAR ALL TREND LINES", g_panelX + 12, g_panelY + 354, 203, 24, C'110,40,40', clrWhite);

   // جداکننده‌ی بصری بین بخش رسم خط روند و بخش سفارش‌های Pending، تا زیاد به هم نچسبند
   CreateRectLabel(SEP_LINE_2, g_panelX + 12, g_panelY + 384, 203, 3, C'150,150,160', C'150,150,160');

   CreateButton(BTN_BUY_PENDING, "BUY PENDING", g_panelX + 12, g_panelY + 392, 98, 24, C'0,90,0', clrWhite);
   CreateButton(BTN_SELL_PENDING, "SELL PENDING", g_panelX + 117, g_panelY + 392, 98, 24, C'130,0,0', clrWhite);

   CreateButton(BTN_CLOSE_BUY, "CLOSE BUYS", g_panelX + 12, g_panelY + 422, 98, 24, C'180,90,0', clrWhite);
   CreateButton(BTN_CLOSE_SELL, "CLOSE SELLS", g_panelX + 117, g_panelY + 422, 98, 24, C'180,90,0', clrWhite);

   CreateButton(BTN_CLOSE_HALF, "CLOSE 50%", g_panelX + 12, g_panelY + 452, 98, 24, C'100,100,0', clrWhite);
   CreateButton(BTN_MANUAL_BE, "MANUAL BE", g_panelX + 117, g_panelY + 452, 98, 24, C'40,100,150', clrWhite);
   
   CreateButton(BTN_CLOSE, "CLOSE ALL", g_panelX + 12, g_panelY + 482, 203, 26, C'70,70,70', clrWhite);

   // ردیف تشخیص روند M5 / M15 / M30 / H1 - دقیقا زیر دکمه CLOSE ALL (Close All bottom edge = panelY + 508)
   int trendRowY = g_panelY + 516;
   ...
   CreateLabel(LBL_BE_TS_STATUS, "No position selected", g_panelX + 12, g_panelY + 540, clrSilver, 8, false);
   CreateButton(BTN_TOGGLE_BE_TS, "TOGGLE BE/TS FOR SELECTED", g_panelX + 12, g_panelY + 556, 203, 22, C'70,70,70', clrWhite);
...
void UpdateBeTsStatusLabel()
{
   if(g_selectedPositionTicket == 0 || !PositionSelectByTicket(g_selectedPositionTicket))
   {
      ObjectSetString(0, LBL_BE_TS_STATUS, OBJPROP_TEXT, "No position selected");
...
// لیست فعلی تیکت‌های هج‌شده را در GlobalVariable ذخیره می‌کند (ماندگار بین ری‌استارت‌های EA/ترمینال)
void PersistHedgedTickets()
{
   int oldCount = 0;
   if(GlobalVariableCheck(g_gvHedgedCountKey)) oldCount = (int)GlobalVariableGet(g_gvHedgedCountKey);
   for(int i = 0; i < oldCount; i++)
      GlobalVariableDel(g_gvHedgedPrefix + IntegerToString(i));

   int newCount = ArraySize(g_hedgedTickets);
   GlobalVariableSet(g_gvHedgedCountKey, (double)newCount);
   for(int i = 0; i < newCount; i++)
      GlobalVariableSet(g_gvHedgedPrefix + IntegerToString(i), (double)g_hedgedTickets[i]);
}
...
void MarkTicketHedged(ulong ticket)
{
   ...
   PersistHedgedTickets(); // بلافاصله ذخیره می‌شود تا بعد از ری‌استارت دوباره هج نشود (رفع باگ)
}

// حذف تیکت‌های بسته‌شده از لیست هج تا آرایه بی‌رویه بزرگ نشود
void CleanupHedgedTickets()
{
   int prevCount = ArraySize(g_hedgedTickets);
   ...
   if(ArraySize(g_hedgedTickets) != prevCount)
      PersistHedgedTickets(); // فقط وقتی لیست واقعا تغییر کرده ذخیره‌سازی انجام شود، نه هر تیک
}
...
void CreateTrendlinePreview(datetime t1, double p1)
{
   ...
   ObjectSetInteger(0, TL_PREVIEW_LINE, OBJPROP_COLOR, clrDodgerBlue);
   ObjectSetInteger(0, TL_PREVIEW_LINE, OBJPROP_STYLE, STYLE_DASH);
   ObjectSetInteger(0, TL_PREVIEW_LINE, OBJPROP_WIDTH, 1);
   ObjectSetInteger(0, TL_PREVIEW_LINE, OBJPROP_RAY_RIGHT, true);
...
void CheckTrendlineTouch()
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      // قیمت «همین الان» خط: فقط برای تشخیص لمس (نزدیک بودن قیمت لحظه‌ای به خط) استفاده می‌شود
      double linePriceNow = ObjectGetValueByTime(0, name, timeCurrent, 0);
      if(linePriceNow <= 0) continue;

      // قیمت خط دقیقا در همان لحظه‌ای که کندل قبلی بسته شده: برای تشخیص جهت (از بالا/پایین آمدن) استفاده می‌شود.
      // رفع باگ: قبلا prevClose (مربوط به زمان گذشته) با linePriceNow (زمان حال) مقایسه می‌شد که برای خطوط با شیب
      // تند می‌توانست جهت را اشتباه تشخیص دهد. حالا هر دو طرف مقایسه‌ی جهت از یک لحظه‌ی زمانی مشترک می‌آیند.
      double linePriceAtPrevBar = ObjectGetValueByTime(0, name, rates[0].time, 0);
      if(linePriceAtPrevBar <= 0) continue;

      bool tradeExecuted = false;

      if(prevClose < linePriceAtPrevBar && bid >= (linePriceNow - tol) && bid <= (linePriceNow + tol))
      {
      ...
      else if(prevClose > linePriceAtPrevBar && bid <= (linePriceNow + tol) && bid >= (linePriceNow - tol))
      {
      ...
      if(tradeExecuted)
      {
         g_tlStates[idx].lastBarTime = currentBarTime; // فقط همین خط علامت‌گذاری می‌شود، نه بقیه خطوط

         // v4.8: خط روی چارت دست‌نخورده باقی می‌ماند (حذف نمی‌شود)؛ فقط کل قابلیت تشخیص خط روند
         // (دکمه EA TREND LINES) خاموش می‌شود تا معامله فعلی بدون مزاحمت مدیریت شود. اگر پوزیشن با
         // سود بسته شد و موقعیت همان خط هنوز معتبر بود، کاربر خودش دوباره دکمه را روشن می‌کند.
         g_trendlineActive = false;
         UpdateTLButtonAppearance();
         SaveVariables();

         Print("Trendline '", name, "' touched -> position opened -> EA Trend Lines toggle turned OFF (line kept on chart).");
         ChartRedraw();
